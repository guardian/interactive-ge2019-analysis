...
const Tooltip = ({ constituency, x, y, ttString }) => {
  const str = constituency?.y2019_hold_gain
  ...
  return (
    <div className='ge-tt' style={{ left: x || 0, top: y - 5 || 0, display: constituency?.name ? 'inline-block' : 'none' }}>
      <div className='ge-tt__winner'><span className={`ge-color--${winner ?.toLowerCase() || 'undeclared'}`}><strong>{(winner) ? winner : 'Undeclared'}</strong></span><span> {rest}</span></div>
        <h3 className='ge-tt__constname'>{constituency?.name}</h3>
        {ttString && ttString !== "NA" &&
